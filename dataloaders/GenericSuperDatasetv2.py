...
class SuperpixelDataset(BaseDataset):
    ...
    def organize_sample_fids(self):
        ...
        for curr_id in self.scan_ids:
            ...
            _real_lb_fid = os.path.join(self.base_dir, f'label_{curr_id}.nii.gz')

            curr_dict["img_fid"] = _img_fid
            curr_dict["lbs_fid"] = _lb_fid
            curr_dict["real_lbs_fid"] = _real_lb_fid
    ...
    def read_dataset(self):
        ...
        for scan_id, itm in self.img_lb_fids.items():
            ...
            # Read real label if exists
            real_lb = None
            if os.path.exists(itm["real_lbs_fid"]):
                real_lb = read_nii_bysitk(itm["real_lbs_fid"])
                real_lb = real_lb.transpose(1,2,0)
                real_lb = np.int32(real_lb)

            img = img[:256, :256, :]
            lb = lb[:256, :256, :]
            
            # Crop real label if exists
            if real_lb is not None:
                real_lb = real_lb[:256, :256, :]
            ...
            out_list.append( {"img": img[..., 0: 1],
                           "lb":lb[..., 0: 0 + 1],
                           "real_lb": real_lb[..., 0: 0 + 1] if real_lb is not None else None,
            ...
            for ii in range(1, img.shape[-1] - 1):
                out_list.append( {"img": img[..., ii: ii + 1],
                           "lb":lb[..., ii: ii + 1],
                           "real_lb": real_lb[..., ii: ii + 1] if real_lb is not None else None,
                           "is_start": False,
                           "is_end": False,
    ...
    def __getitem__(self, index):
        ...
        real_lb_raw = curr_dict.get("real_lb", None)
        ...
        # Priority: use real label if exists, otherwise use pseudolabel
        if real_lb_raw is not None:
            # Check if real label contains any foreground
            if np.max(real_lb_raw) > 0:
                # Select a random foreground class from real labels
                unique_classes = np.unique(real_lb_raw)
                foreground_classes = [c for c in unique_classes if c > 0]
                if len(foreground_classes) > 0:
                    selected_class = np.random.choice(foreground_classes)
                    label_t = np.float32(real_lb_raw == selected_class)
                else:
                    # No foreground in real label, use pseudolabel
                    label_t = self.supcls_pick_binarize(label_raw, sup_max_cls)
            else:
                # No foreground in real label, use pseudolabel
                label_t = self.supcls_pick_binarize(label_raw, sup_max_cls)
        else:
            # Real label doesn't exist, use pseudolabel
            label_t = self.supcls_pick_binarize(label_raw, sup_max_cls)
